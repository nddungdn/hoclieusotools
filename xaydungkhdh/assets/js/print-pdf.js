...
export function exportPdfViaPrint(state){
  ...
  *{box-sizing:border-box}body{font-family:"Times New Roman",serif;font-size:13pt;line-height:1.15;color:#000;margin:0}
  ...
  table{border-collapse:collapse;width:100%;table-layout:fixed;margin:5pt 0 10pt;page-break-inside:auto}tr{page-break-inside:avoid}th,td{border:1px solid #000;padding:4px 5px;vertical-align:top;font-size:11pt;overflow-wrap:anywhere}th{text-align:center;font-weight:700}
...
function renderBlock(b){
  ...
  if(b.type==='subheading')return `<h2 class="center">${escapeHtml(b.text)}</h2>`;
  if(b.type==='paragraph')return `<p>${escapeHtml(b.text).replace(/\n/g,'<br>')}</p>`;
  if(b.type==='table'){const cols=Array.isArray(b.widths)&&b.widths.length===b.headers.length?`<colgroup>${b.widths.map(w=>`<col style="width:${Number(w)||0}%">`).join('')}</colgroup>`:'';return `<table>${cols}<thead><tr>${b.headers.map(x=>`<th>${escapeHtml(x)}</th>`).join('')}</tr></thead><tbody>${(b.rows.length?b.rows:[Array(b.headers.length).fill('')]).map(r=>`<tr>${b.headers.map((_,i)=>`<td>${escapeHtml(r[i]??'').replace(/\n/g,'<br>')}</td>`).join('')}</tr>`).join('')}</tbody></table>`;}
